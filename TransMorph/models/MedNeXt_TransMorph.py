...
class down_block(nn.Module):
    def __init__(self, c_in,out_channels, scale=16, k_size=3):
        ...
        self.dw_conv = nn.Conv3d(c_in, c_in, kernel_size=k_size, groups=c_in, stride=2, padding=(k_size - 1) // 2,
                                 bias=False)
        self.norm = nn.GroupNorm(16, c_in)
...
class DecoderBlock2(nn.Module):
    def __init__(
        ...
        self.expansion = nn.Conv3d(in_channels, in_channels * 4, kernel_size=(1, 1, 1), stride=1)
        self.act = nn.GELU()
        self.compress = nn.Conv3d(in_channels * 4, in_channels, kernel_size=(1, 1, 1), stride=1)
        self.shortcut = nn.ConvTranspose3d(in_channels, in_channels, kernel_size=(1, 1, 1), output_padding=1, stride=2)
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.conv1 = conv_block(config.in_chans)
        self.down1 = down_block(config.in_chans, config.in_chans)
